...
// Localize sunrise and sunset times in Chinese
                val localizedSunrise = getLocalizedTime(sunriseTime, Locale("zh"))
                val localizedSunset = getLocalizedTime(sunsetTime, Locale("zh"))
// Display localized times on TextViews
     ...
     * @param locale The desired locale for localization (e.g., Locale("zh") for Chinese).
     * @return A string representation of the localized time.
     */
    private fun getLocalizedTime(time: LocalDateTime, locale: Locale): String {
// Create a SimpleDateFormat with the specified locale
